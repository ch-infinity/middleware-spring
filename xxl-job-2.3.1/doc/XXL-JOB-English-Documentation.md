...
- 25、Event trigger：In addition to "Cron" and "Task Dependency" to trigger tasks, support event-based triggering tasks. The dispatch center provides API com.ch.commoninterface.service that triggers a single execution of the task, it can be triggered flexibly according to business events. 
...
xxl-job-admin can be visited through nginx proxy and configure a com.ch.commoninterface.domain for nginx,and the com.ch.commoninterface.domain url can be configured as the executor’s callback url.
...
<context:component-scan base-package="com.xxl.job.executor.com.ch.commoninterface.service.jobhandler" />
...
This call module is like RPC,RemoteHttpJobBean provide call proxy functionality,the executor is provided as remote com.ch.commoninterface.service.
...
he reason may be:restart com.ch.commoninterface.service,schedule thread was blocked by QuartzJobBean, threads was exhausted,some task enable @DisallowConcurrentExecution，the last schedule  was blocked and next schedule was missed.
...
#### 5.4.7 log callback com.ch.commoninterface.service
When schedule center of the schedule module was deployed as web com.ch.commoninterface.service, on one side it play as schedule center, on the other side it also provide api com.ch.commoninterface.service for executor. 

The source code location of schedule center’s “log callback api com.ch.commoninterface.service” as shown below:
```
...
Development steps:go and see "chapter 3" .
Principle : every "GLUE模式(Java)" task code is a class implemets interface IJobHandler, when executor received schedule request from schedule center these code will be loaded by Groovy classloader and instantiate into a Java object and inject spring bean com.ch.commoninterface.service declared in this code at the same time（please confirm com.ch.commoninterface.service and class reference in Glue code exist in executor project）, then call the object’s execute() method and execute task logic.
...
When executor received schedule request from schedule center, if task type is “Bean模式” it will match bean mode task in Spring container and call it’s execute() method and execute task logic. if task type is “GLUE模式”, it will load Glue code, instantiate a Java object and inject other spring com.ch.commoninterface.service（notice: the spring com.ch.commoninterface.service injected in Glue code must exist in the same executor project）, then call execute() method and execute task logic. 
...
#### 5.6.1 A complete task schedule communication process
    - 1,schedule center send http request to executor, and the com.ch.commoninterface.service in executor in fact is a jetty server with default port 9999.
    - 2,executor execute task logic.
    - 3,executor http callback with schedule center for schedule result, the com.ch.commoninterface.service in schedule center used to receive callback request from executor is a set of api opended to executor.
...
The scheduling center provides API services for executors and business parties to choose to use, and the currently available API services are available.
    ...
    1. Job result callback com.ch.commoninterface.service;
    2. Executor registration com.ch.commoninterface.service;
    3. Executor registration remove services;
    4. Triggers a single execution com.ch.commoninterface.service, and support the task to be triggered according to the business event;

The scheduling center API com.ch.commoninterface.service location: com.xxl.job.core.biz.AdminBiz.java

The scheduling center API com.ch.commoninterface.service requests reference code：com.xxl.job.adminbiz.AdminBizTest.java
...
- 3、com.ch.commoninterface.service HA:task info stored in mysql, Job com.ch.commoninterface.service support cluster to make sure com.ch.commoninterface.service HA;
...
- 2、optimize API callback com.ch.commoninterface.service failed status code，facilitate troubleshooting;
...
- 6、schedule Center API com.ch.commoninterface.service change to self-study RPC framework to u nify communication model;
- 7、add schedule center API com.ch.commoninterface.service test Demo, convenient in dispatch center API extension and testing;
