        ...
        for (Map.Entry<String, String[]> entry : params.entrySet()) {
            if (entry.getKey() != null && entry.getKey().length() > 0
                ...
                if (!currentUser.hasServicePrivilege(entry.getKey())) {
                    throw new IllegalStateException("The user " + currentUser.getUsername() + " have no privilege of com.ch.commoninterface.service " + entry.getKey());
                }
                ...
                for (Map.Entry<String, String> e : url2query.entrySet()) {
                    ...
                    if (StringUtils.isBlank(app)) {
                        throw new IllegalStateException("No application for com.ch.commoninterface.service(" + serviceName + "): "
                                + e.getKey() + "?" + e.getValue());
                    }
                }
