...
public class RegistryServerSync implements InitializingBean, DisposableBean, NotifyListener {
    ...
    // Notification of of any com.ch.commoninterface.service with any type (override、subcribe、route、provider) is full.
    public void notify(List<URL> urls) {
        ...
        for (Map.Entry<String, Map<String, Map<Long, URL>>> categoryEntry : categories.entrySet()) {
            ...
            } else {// Fix map can not be cleared when com.ch.commoninterface.service is unregistered: when a unique “group/com.ch.commoninterface.service:version” com.ch.commoninterface.service is unregistered, but we still have the same services with different version or group, so empty protocols can not be invoked.
