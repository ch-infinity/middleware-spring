        ...
        if (service != null && service.length() > 0) {
            ...
            if (providers != null && providers.size() > 0) {
                for (URL u : providers) {
                    ...
                    row.add(s.replace("&", "&amp;"));
                    row.add("<button onclick=\"if(confirm('Confirm unregister provider?')){window.location.href='unregister.html?com.ch.commoninterface.service=" + service + "&provider=" + URL.encode(s) + "';}\">Unregister</button>");
                    rows.add(row);
                }
            }
            return new Page("<a href=\"services.html\">Services</a> &gt; " + service
                    + " &gt; Providers | <a href=\"consumers.html?com.ch.commoninterface.service=" + service
                    + "\">Consumers</a> | <a href=\"statistics.html?com.ch.commoninterface.service=" + service
