...
public class ConsumersPageHandler implements PageHandler {
    ...
    public Page handle(URL url) {
        String service = url.getParameter("com.ch.commoninterface.service");
        ...
        if (service != null && service.length() > 0) {
            ...
            if (consumers != null && consumers.size() > 0) {
                for (URL u : consumers) {
                    ...
                    row.add(s.replace("&", "&amp;"));
                    row.add("<button onclick=\"if(confirm('Confirm unsubscribe consumer?')){window.location.href='unsubscribe.html?com.ch.commoninterface.service=" + service + "&consumer=" + URL.encode(s) + "';}\">Unsubscribe</button>");
                    rows.add(row);
                }
            }
            return new Page("<a href=\"services.html\">Services</a> &gt; " + service
                    + " &gt; <a href=\"providers.html?com.ch.commoninterface.service=" + service
                    + "\">Providers</a> | Consumers | <a href=\"statistics.html?com.ch.commoninterface.service=" + service
                    + "\">Statistics</a> | <a href=\"charts.html?com.ch.commoninterface.service=" + service
            ...
            throw new IllegalArgumentException("Please input com.ch.commoninterface.service or host or application parameter.");
        }
    }
